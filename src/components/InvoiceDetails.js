...
export const InvoiceDetails = () => {
  ...
  const subtotal = data.products.reduce((acc, product) => acc + product.amount, 0);
  ...
  const sendEmailWithInvoice = () => {
    ...
    html2canvas(input).then((canvas) => {
      ...
      uploadString(storageRef, imageData, "data_url")
        ...
        .then((downloadURL) => {
          // Use the download URL in the email
          const emailParams = {
            to_email: data.billedTo.email, // recipient email
          ...
          emailjs.send(
            "service_j4wewo5", // replace with your EmailJS service ID
            "template_ori725j", // replace with your EmailJS template ID
            emailParams,
            "ErJHgIM8qLrffa_RT" // replace with your EmailJS user ID
          ).then((response) => {
  ...
  return (
    <div >
      <div className="invoice-top-header">
        ...
        <button onClick={sendEmailWithInvoice} className="email-btn">Send Email</button>
      </div>
    <div ref={invoiceRef} className="p-6 max-w-4xl mx-auto border border-black bg-white ">
      <div className="border border-black">
        ...
        <img
            ...
            />
          <span className="font-bold text-lg mx-2">GSTIN:{data.companyGstin}</span>
          <p>Original Copy</p>
        </div>
        <div className="text-center mb-6 ">
          <h1 className="text-xm font-bold underline uppercase">Tax Invoice</h1>
          <span className="font-bold text-2xl m-5"> {data.companyName}</span>
          <p></p>
          <span className="font-bold"> {data.companyAddress}</span> 
          <p></p>
          <span className="font-bold">Emali:- {data.companyEmail} +91{data.companyContact}</span>
        </div>
      </div>
      ...
      <table className="w-full">
        <thead>
          <tr className="bg-gray-100">
            <th className="border-x border-b border-black px-6 py-2 text-left w-5">S.N</th>
            <th className="border-r border-b border-black px-6 py-2 text-left ">{data.yourInvoise}</th>
            <th className="border-r border-b border-black px-6 py-2 w-96 text-center">Amount</th>
          </tr>
        </thead>
        <tbody>
          {data.products.map((product, index) => (
            <tr key={index}>
              <td className="border-x border-b border-black px-6 py-2">{index + 1}</td>
              <td className="border-r border-b border-black px-6 py-2">{product.resource}</td>
              <td className="border-r border-b border-black px-6 py-2 text-center">₹{product.amount}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
          <tr>
            <td colSpan="2" className="border-x border-b border-black px-6 py-2 text-right">
              SubTotal
            </td>
            <td className="border-r border-b border-black px-6 py-2 text-center">₹{subtotal}</td>
          </tr>
          <tr>
            <td colSpan="2" className="border-x border-b border-black px-6 py-2 text-right">
              GST
            </td>
            <td className="border-r border-b border-black px-6 py-2 text-center">₹{data.totalTax}</td>
          </tr>
          <tr>
            <td
              colSpan="2"
  ...
  <p className="font-semibold">
    {numberToWords(grandTotal)}
            ...
            <span className="flex items-start flex-col font-bold">Grand Total</span> 
              ₹{grandTotal}
            </td>
          </tr>
        </tfoot>
